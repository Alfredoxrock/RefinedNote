...
try {
    ...
    if (-not $isAdmin) {
        Write-Host "⚠️  For best results, run this script as Administrator to install AWS CLI system-wide." -ForegroundColor Yellow
    ...
    try {
        ...
        $awsVersion = aws --version 2>$null
        if ($awsVersion) {
            Write-Host "✓ AWS CLI installed successfully: $awsVersion" -ForegroundColor Green
        } else {
            Write-Host "❌ AWS CLI installation failed or not in PATH. You may need to restart PowerShell." -ForegroundColor Red
...
try {
    ...
    if ($identity) {
        Write-Host "✓ AWS credentials configured. Using account: $($identity.Account), user/role: $($identity.Arn)" -ForegroundColor Green
    } else {
    ...
    try {
        ...
        Write-Host "✓ AWS credentials configured successfully. Account: $($identity.Account)" -ForegroundColor Green
    } catch {
...
if (-not (Test-Path $LocalExePath)) {
    Write-Host "❌ Installer file not found at: $LocalExePath" -ForegroundColor Red
...
Write-Host "✓ Found installer: $LocalExePath ($([math]::Round($fileSize / 1MB, 2)) MB)" -ForegroundColor Green
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✓ Upload successful (public)" -ForegroundColor Green
        $uploadedPublic = $true
    } else {
        Write-Host "⚠️  Public upload failed (possibly due to account restrictions). Trying private upload..." -ForegroundColor Yellow
        # Try private upload
        aws s3 cp $LocalExePath "s3://$Bucket/$Key" --content-type 'application/octet-stream'
        
        if ($LASTEXITCODE -eq 0) {
            Write-Host "✓ Upload successful (private)" -ForegroundColor Green
...
if ($uploadedPublic) {
    try {
        ...
        Write-Host "✓ Public URL accessible (HTTP $($response.StatusCode))" -ForegroundColor Green
        $publicAccessible = $true
    } catch {
        Write-Host "⚠️  Public URL not accessible: $($_.Exception.Message)" -ForegroundColor Yellow
...
try {
    ...
    if ($presignedUrl -and $LASTEXITCODE -eq 0) {
        Write-Host "✓ Presigned URL generated (valid for $PresignExpireHours hours)" -ForegroundColor Green
    } else {
        Write-Host "⚠️  Presigned URL generation failed" -ForegroundColor Yellow
        $presignedUrl = $null
    }
} catch {
    Write-Host "⚠️  Presigned URL generation failed: $($_.Exception.Message)" -ForegroundColor Yellow
    $presignedUrl = $null
}
...
if ($publicAccessible) {
    Write-Host "✅ USE THIS PUBLIC URL IN PARTNER CENTER:" -ForegroundColor Green
    Write-Host $publicUrl -ForegroundColor Cyan
    Write-Host ""
    Write-Host "This URL is publicly accessible and can be used directly in Partner Center." -ForegroundColor White
} elseif ($presignedUrl) {
    Write-Host "✅ USE THIS PRESIGNED URL IN PARTNER CENTER:" -ForegroundColor Green
    ...
    Write-Host "❌ No accessible URL available. Check your bucket policies and permissions." -ForegroundColor Red
}
